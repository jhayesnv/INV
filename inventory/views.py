...
def get_well_pours():
    well_pours = SpiritOrderItem.objects.filter(is_well=True)
    return well_pours


def get_glass_pours():
    glass_pours = WineOrderItem.objects.filter(is_glass_pour=True)
    return glass_pours


def get_draft_pours():
    draft_pours = BeerOrderItem.objects.filter(format='Draft')
    return draft_pours
